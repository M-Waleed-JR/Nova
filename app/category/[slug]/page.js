...
export async function generateStaticParams() {
  // If you use static generation for categories; otherwise leave empty for dynamic
  return [];
}
  ...
  return (
    <main className="min-h-screen bg-[#030308] text-white">
      <div className="mx-auto max-w-7xl px-4 py-8 md:px-12 md:py-12">
        <Link
          href={`/category/${product.category || "phones"}`}
        ...
        <div className="grid gap-8 md:grid-cols-2 md:gap-12">
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#05050b]">
            <Image
              src={product.image || product.coverImage || "/logo/withOutBackground.png"}
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-4 flex items-baseline gap-3">
              ...
              {product.oldPrice && (
                <del className="text-lg text-white/40">${product.oldPrice}</del>
              )}
            </div>
            ...
            <div className="mt-6 flex items-center gap-6 text-xs text-white/50">
              ...
              <span className="flex items-center gap-1.5">
                <ShieldCheck className="h-3.5 w-3.5 text-cyan-400" /> Guarantee
              </span>
            </div>
          </div>
        </div>
